...
// Importa Web3.js y configura la conexión con la blockchain
...
async function mintNFT() {
    ...
    try {
        await contract.methods.mintWithPassword(account[0], password).send({ from: account[0] });
        alert('NFT minteado exitosamente!');
        updateMintCount();
    } catch (error) {
        console.error("Error al mintear NFT: ", error);
        alert('Error al mintear NFT. Verifica la contraseña y vuelve a intentarlo.');
    }
}

// Actualizar el contador de NFTs minteados
async function updateMintCount() {
    try {
        const totalMinted = await contract.methods.totalSupply().call();
        const maxSupply = 100; // Actualiza esto si tienes un número diferente
        document.getElementById('mint-count').textContent = `Total Minteados: ${totalMinted} / ${maxSupply}`;
    } catch (error) {
        console.error("Error al obtener el total de NFTs minteados: ", error);
    }
}
...
// Llama la función para actualizar el contador al cargar la página
updateMintCount();
